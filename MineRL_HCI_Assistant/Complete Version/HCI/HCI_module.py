...
class HCI_controller:
 ...
 def __init__(self, task, include_basic_actions=False, basic_action_delay=0):
  """
  The class will be initialzed by steps below.
  1.Load the chain
  ...
  if self.include_basic_actions:
    if self.task == 'iron_pickaxe':
      raise Exception('when task is iron_pickaxe, you cannot let include_basic_actions=True')
    elif self.task == 'stone_pickaxe':
      ...
      raise Exception('task is error! task should be one of [\'iron_pickaxe\', \'stone_pickaxe\']')


 def load_chain(self):
  """
  goal: load chain given task.
  """
  if self.task == 'iron_pickaxe':
    chain_path = './HCI/ironpickaxe_chain.json'
  elif self.task == 'stone_pickaxe':
    chain_path = './HCI/stonepickaxe_chain.json'
  else: 
    raise Exception('task is error! task should be one of [\'iron_pickaxe\', \'stone_pickaxe\']')
