...
class Agent:
    ...
    def give_basic_action_recommendation(self, state, epsilon=0.01):
        # because the HDQfD Agents accept state after the Frame_stack, now we simply stack the state together
        #assert state.shape == (64,64,3)
        nn_input = np.dstack((state,state)).reshape(1,64,64,6)
        q_value = self.online_model(nn_input, training=False)
        if random.random() <= epsilon:
