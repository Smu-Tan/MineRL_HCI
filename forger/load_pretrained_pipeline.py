...
def run_pipeline(config):
    ...
    wandb.init(config=tf.compat.v1.flags.FLAGS, sync_tensorboard=True, anonymous='allow', project="Treechop",
               group='PC_load_pretrained_pipeline')
    ...
    print('\n==========================pipeline step3: load pre-trained model==========================')

    print(agent.save_dir)
    agent.load(os.path.join(agent.save_dir, "pre_trained_model.ckpt"))

    print('\npre-trained loaded')

    print('\n==========================pipeline step4: train agent in treechop env==========================')
    env = wrap_env(gym.make("MineRLTreechop-v0"), **wrappers_config)
    summary_writer = tf.summary.create_file_writer('train/tfboard/log/load_pretrained_log_agent_from_MPI/Treechop_set/')
    with summary_writer.as_default():
        scores_, _ = agent.train(env, name="model.ckpt", **pipeline_config['treechop'])
    env.close()

    print('\n==========================pipeline step4: train agent in pickaxe env==========================')
    env = wrap_env(gym.make("MineRLObtainIronPickaxeDense-v0"),
                   **wrappers_config)
    summary_writer = tf.summary.create_file_writer('train/tfboard/log/load_pretrained_log_agent_from_MPI/Pickaxe_set/')
    with summary_writer.as_default():
